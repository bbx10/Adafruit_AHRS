...
void setup()
{
  ...
  // TCP client setup.
  tcpClient = new Client(this, "esp_ahrs.local", 9999);
  if (tcpClient != null) println("TCP socket open");
...
void draw()
{
  ...
  // Process row, pitch, yaw from network
  netEvent();
...
void netEvent()
{
  ...
  while (tcpClient.available() > 0) {
    aLine = tcpClient.readStringUntil('\n');
    if (aLine == null) break;
    
    if (printSerial) print(aLine);
    if (aLine.length() > 8) {
      String[] list = split(aLine, " ");
      if ( (list.length >= 4) && (list[0].equals("Orientation:")) ) 
      {
        ...
        yaw   = float(list[3]);
        buffer = aLine;
      } else if ( (list.length >= 2) && (list[0].equals("Alt:")) ) 
      {
        alt  = float(list[1]);
        buffer = aLine;
      } else if ( (list.length >= 2) && (list[0].equals("Temp:")) ) 
      {
        temp  = float(list[1]);
        buffer = aLine;
      }
